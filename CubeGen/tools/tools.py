...
def band_cube(spec,age,name,crval,cdelt,crpix,hdr,dir='',dir1='',outs=0,fitsf=0,zrf=0.1,clam=0.9):
    ...
    file=['SDSS_u.txt','SDSS_g.txt','SDSS_r.txt','SDSS_i.txt','SDSS_z.txt','NUV_GALEX.txt','FUV_GALEX.txt','U_Johnson.txt','B_Johnson.txt','V_Johnson.txt','I_Cousins.txt','R_Cousins.txt','J_2MASS.txt','H_2MASS.txt','K_2MASS.txt','ha_filter_sh.txt']
    ...
    if outs == 1:
        ...
        if len(pdl_00r[np.where(pdl_00r > 0)]) > 0:
            max=24.0#np.nanmin(pdl_00r[np.where(pdl_00r > 0)])-0.5#24.0#17.5
        else:
            max=20.0
        min=25.0
        nx,ny=pdl_00g.shape
