...
def rssp_multi1d(namelist,path='./',path_in='./',path_out='./',hid='RA',sep=',',key_files='FILE',keydir='ID',keyred='Z',
    ...
    if len(keys_ext) > 0:
        for i in range(len(keys_ext)):
            fits_new_cols.extend([fits.Column(name=new_keys_ext[i], format=tools.numpy_to_tform(new_values[i]), array=new_values[i])])
    coldefs = fits.ColDefs([col1, col2, col3, col4, col5, col6, col7, col8, col9, 
                            col10, col11, col12, col13, col14, col15, col16, col17, col18, col19, col20, col21, *fits_new_cols])
    h11 = fits.BinTableHDU.from_columns(coldefs)
    h_y=h11.header
    h_y['EXTNAME']='SLITMAP'
    h_y.update()
